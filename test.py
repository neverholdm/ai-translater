...
# 导入项目模块前屏蔽 .env 加载；测试只使用临时的假 Key。
...
def make_status_error(error_type, status_code):
    ...
    return error_type("模拟 API 错误", response=response, body=None)
...
class TestTranslate(unittest.TestCase):
    ...
    def test_text_too_long(self):
        with patch.dict(
            ...
            with self.assertRaises(TranslationInputError):
                translate("a" * (MAX_TEXT_LENGTH + 1), "中文", "英语")
    ...
    def test_success_and_timeout(self):
        with patch.dict(
            ...
            with patch("translate.OpenAI") as mock_openai:
                ...
                self.assertEqual(
                    mock_openai.call_args.kwargs["timeout"], REQUEST_TIMEOUT
                )
    ...
    def test_authentication_error(self):
        self.assert_request_error(
            make_status_error(AuthenticationError, 401), TranslationAuthError
        )

    def test_timeout_error(self):
        request = httpx2.Request("POST", "https://api.deepseek.com")
        self.assert_request_error(
            APITimeoutError(request), TranslationNetworkError
        )

    def test_connection_error(self):
        request = httpx2.Request("POST", "https://api.deepseek.com")
        self.assert_request_error(
            APIConnectionError(request=request), TranslationNetworkError
        )

    def test_rate_limit_error(self):
        self.assert_request_error(
            make_status_error(RateLimitError, 429), TranslationRateLimitError
        )

    def test_server_error(self):
        self.assert_request_error(
            make_status_error(APIStatusError, 500), TranslationServerError
        )


class TestCli(unittest.TestCase):
    def test_failure_does_not_save_history(self):
        with patch("builtins.input", side_effect=["中文", "英语", "你好"]):
            with patch(
                "main.translate", side_effect=TranslationNetworkError("网络错误")
            ):
                with patch("main.save_translate_history") as mock_save:
    ...
    def test_save_history(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            try:
                ...
                with open("translation_history.txt", "r", encoding="utf-8") as file:
                    content = file.read()
            finally:
